...
/**
 * Class that defines command for changing airplane mode state.
 */
public class AirplaneModeContentChangeCommand extends ContentStateChangeServiceCommandTemplate {

    private static final String TAG = LogUtil.getTag(AirplaneModeContentChangeCommand.class);
    ...
    public void doBeforeToggle(Context context) {
        ...
        analytics.sendEvent(FlurryCustomEvents.ON_AIRPLANE_MODE_STATE_CHANGED, true);
    }

    @Override
    public void performToggle(Context context) {
        LogUtil.d(TAG, "Called performToggle");
        boolean enabled = !AirplaneModeUtility.isAirplaneModeOn(context);
        AirplaneModeUtility.setAirplaneMode(context, enabled);

        AnalyticsInterface analytics = AnalyticsFactory.get(AnalyticsProviderType.FLURRY_ANALYTICS);
        analytics.sendEvent(FlurryCustomEvents.ON_AIRPLANE_MODE_STATE_CHANGED,
                ContentType.AIRPLANE_MODE.toString(), enabled);
    }
