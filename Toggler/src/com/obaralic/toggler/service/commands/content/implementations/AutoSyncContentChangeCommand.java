...
/**
 * Class that defines command for changing auto sync state.
 */
public class AutoSyncContentChangeCommand extends ContentStateChangeServiceCommandTemplate {

    private static final String TAG = LogUtil.getTag(AutoSyncContentChangeCommand.class);
    ...
    public void doBeforeToggle(Context context) {
        ...
        analytics.sendEvent(FlurryCustomEvents.ON_AUTO_SYNC_STATE_CHANGED, true);
    }
    ...
    public void performToggle(Context context) {
        ...
        int autoSyncStatus = statusesArray[contentId];

        boolean isEnabled = !AutoSyncUtility.isEnabled(context);
        int isEnabledInt = isEnabled ? AutoSyncUtility.AUTO_SYNC_ENABLED : AutoSyncUtility.AUTO_SYNC_DISABLED;
       
        if (isEnabledInt != autoSyncStatus) {
            statusesArray[contentId] = isEnabledInt;
            ContentStatesPersistence.getInstance().setContentStates(context, statusesArray);
        }

        AutoSyncUtility.setEnabled(context, isEnabled);
        WidgetUiServiceFacade.get().startForSimpleRedraw(context);
                
        AnalyticsInterface analytics = AnalyticsFactory.get(AnalyticsProviderType.FLURRY_ANALYTICS);
        analytics.sendEvent(FlurryCustomEvents.ON_AUTO_SYNC_STATE_CHANGED,
                ContentType.AUTO_SYNC.toString(), isEnabled);
    }
