...
/**
 * Class that defines command for changing ringer mode state.
 */
public class RingModeContentChangeCommand extends ContentStateChangeServiceCommandTemplate {

    private static final String TAG = LogUtil.getTag(RingModeContentChangeCommand.class);
    ...
    public void doBeforeToggle(Context context) {
        ...
        analytics.sendEvent(FlurryCustomEvents.ON_RINGER_STATE_CHANGED, true);
    }
    ...
    public void performToggle(Context context) {
        ...
        RingerModeUtitliy.setRingerMode(context, ringerMode);
        
        AnalyticsInterface analytics = AnalyticsFactory.get(AnalyticsProviderType.FLURRY_ANALYTICS);
        analytics.sendEvent(FlurryCustomEvents.ON_RINGER_STATE_CHANGED,
                ContentType.RINGER_MODE.toString(), RingerModeUtitliy.valueOf(ringerMode));
    }
