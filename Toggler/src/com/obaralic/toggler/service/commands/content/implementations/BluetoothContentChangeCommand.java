...
/**
 * Class that defines command for changing bluetooth state.
 */
public class BluetoothContentChangeCommand extends ContentStateChangeServiceCommandTemplate {

    private static final String TAG = LogUtil.getTag(BluetoothContentChangeCommand.class);
    ...
    public void doBeforeToggle(Context context) {
        ...
        analytics.sendEvent(FlurryCustomEvents.ON_BLUETOOTH_STATE_CHANGED, true);
    }
    ...
    public void performToggle(Context context) {
        ...
        int bluetoothStatus = statusesArray[contentId];

        boolean enable = !(bluetoothStatus == BluetoothUtility.BLUETOOTH_ENABLED || bluetoothStatus == BluetoothUtility.BLUETOOTH_ENABLING);

        Intent intent = new Intent();
        intent.setAction(BluetoothUtility.ACTION_CHANGE_BLUETOOTH_STATE);
        intent.putExtra(BluetoothUtility.EXTRA_CHANGE_BLUETOOTH_STATE, enable);
