...
/**
 * Class that defines command for changing WiFi hotspot state.
 */
public class WiFiHotspotContentChangeCommand extends ContentStateChangeServiceCommandTemplate {

    private static final String TAG = LogUtil.getTag(WiFiHotspotContentChangeCommand.class);
    ...
    public void doBeforeToggle(Context context) {
        ...
        analytics.sendEvent(FlurryCustomEvents.ON_WIFI_HOTSPOT_STATE_CHANGED, true);
    }

    @Override
    public void performToggle(Context context) {
        LogUtil.d(TAG, "Called performToggle");
        boolean enabled = !WiFiHotspotUtility.isEnabled(context);
        WiFiHotspotUtility.setWifiApEnabled(context, enabled);
        
        AnalyticsInterface analytics = AnalyticsFactory.get(AnalyticsProviderType.FLURRY_ANALYTICS);
        analytics.sendEvent(FlurryCustomEvents.ON_WIFI_HOTSPOT_STATE_CHANGED,
                ContentType.WIFI_HOTSPOT.toString(), enabled);
    }
